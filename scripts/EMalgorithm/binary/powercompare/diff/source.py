		...
		for i1 in ['0','1','1.5']:
			for i2 in ['0','0.5','1']:
				for i3 in ['200']:
					...
					RFile.write('simuresdiff=power_comparediff(binary=TRUE,Nite=Nsim,sgdata=sgdata,bmrpars=log(BMR),Nsample=i3,betaf0=i1,beta_gc=c(i2,2.2),para=c(0.8,0.2),hotseq=hotseq1,hmm=hmm)\n')
					...
					shFile.write('#SBATCH --job-name=base'+i1+'_'+i2+'_'+i3+'\n')
					shFile.write('#SBATCH --nodes=1\n')
					shFile.write('#SBATCH --ntasks-per-node=1\n')
					shFile.write('#SBATCH --time=00:00:20\n')
