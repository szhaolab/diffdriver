		...
		for i1 in ['0','1','1.5']:
			for i2 in ['0','0.5','1']:
				for i3 in ['200','400','600','800']:
						...
						RFile.write('simuresother=vector("list",200)\n')
						RFile.write('for (i4 in c(1:200)){\n')                			
						RFile.write('simuresother[[i4]]=power_compareother(binary=TRUE,Nite=Nsim,sgdata=sgdata200[[i4]],bmrpars=log(BMR),Nsample=i3,betaf0=i1,beta_gc=c(i2,2.2),para=c(0.8,0.2),hotseq=hotseq0,hmm=hmm)\n')
						RFile.write('}\n')
						RFile.write('save(simuresother,file="'+outfiles +'")\n')
						RFile.write('q("no")\n')
						RFile.close()
						shFileName = 'phynotype'+identifier  +'.sh'
						shFile = open(shFileName, 'w')
						shFile.write('#!/bin/bash\n')
						shFile.write('#SBATCH --job-name=other'+i1+'_'+i2+'_'+i3+'\n')
						shFile.write('#SBATCH --nodes=1\n')
						shFile.write('#SBATCH --ntasks-per-node=1\n')
						shFile.write('#SBATCH --time=20:00:00\n')
