...
#'
#' @examples
simulate_dev <- function(gene,funcanno,Afiledir,binary,  para,Nsample, betaf0,  beta_gc){
  if (binary==T){ # generate binary phenotype
    Nsamplec <- round(Nsample/2) # number of samples with phenotype E=1 (the rest will be 0)
    Nsamplen <- Nsample-Nsamplec
    phenotype <- c(rep(1,Nsamplec),rep(0,Nsamplen))
  ...
  annodata <- list() # a list of nine annotation data frames
  bmrmtxlist <- list() # a list of nine background mutation matrices
  rate=mutp(gene,funcanno,Afiledir,exp(betaf0),  exp(beta_gc))
  mutlist <- vector("list",9) # a list of nine mutation matrices
  for (t in 1:length(rate)) {
    anno=rate[[t]]
    tnpos1 <- dim(anno[functypecode==7,])[1]
    tnpos2 <- dim(anno[functypecode==8,])[1]
    mutlist[[t]]=rsparsematrix(nrow = 1,ncol = Nsample,density = 0)
    for (i in 1:nrow(anno)) {
      pp1=anno[i,bmr]
      pp2=anno[i,pmr]
      mutj1= rsparsematrix(1,Nsample.ps,nnz = rbinom(1, Nsample.ps, pp1),rand.x=NULL)
      mutj2= rsparsematrix(1,Nsample.neu,nnz = rbinom(1, Nsample.neu, pp2),rand.x=NULL)
      mutj=cbind(mutj1,mutj2)
      mutlist[[t]]=rbind(mutlist[[t]],mutj)
    }
    mutlist[[t]]=mutlist[[t]][-1,]
    countlist[[t]] <- c(tnpos1, tnpos2)
    bmrmtxlist[[t]] <- matrix(anno[1,bmr], ncol = ncol(mutlist[[t]]), nrow = nrow(mutlist[[t]])) # background mutation matrix for nytype=t
  ...
  pos1pos2ratio <- colSums(do.call(rbind, countlist))[1]/colSums(do.call(rbind, countlist))[2]
  avbetaf1f2 <- log((pos1pos2ratio*exp(avbetaf1) + exp(avbetaf2))/(pos1pos2ratio+1))
  simdata <- list("mutlist"= mutlist, "pheno" = phenotype, "annodata" = rate, "bmrmtxlist" = bmrmtxlist, "para"=para, "efsize" = list( "betaf0" = betaf0,  "beta_gc" = beta_gc, "avbetaf1" = avbetaf1, "avbetaf2" = avbetaf2, "avbetaf1f2" = avbetaf1f2),"nsample"=c(Nsample.ps,Nsample.neu))
  return(simdata)
}
