...
power_compare <- function(family, Niter, sgdata, Nsample,para,bmrpars,betaf0,beta_gc,hotspot){
  ...
  for (iter in 1:Niter) {
    ...
    if (family=="binary"){
      simdata <- simulate_1funcv(sgdata=sgdata, bmrpars=bmrpars, betaf0=betaf0, Nsample=Nsample, beta_gc=beta_gc, para=para,hotspot=hotspot)
    }else{
      simdata <- simulate_2funcv(sgdata=sgdata, bmrpars=bmrpars, betaf0=betaf0, Nsample=Nsample, beta_gc=beta_gc, para=para,hotspot=hotspot)
    }
