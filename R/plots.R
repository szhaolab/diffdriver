...
plot_mut <- function(gene_name, mut, pheno, totalnttype =96, anno_dir = ".", output_prefix = "plot", output_dir = "."){
  ...
  cdata <- prep_pheno_mut_data(mut, pheno, add_dt = nmutdt)
  mut <-cdata$mut
  ci <- cdata$ci
  pheno <- cdata$pheno
  ...
  df <-t(cbind(pheno[[cdata$phename]], colSums(mutmtx), colSums(mutmtx[fanno$functypecode8>0,]), colSums(mutmtx[fanno$mycons>0,]), pheno$Nmut))
