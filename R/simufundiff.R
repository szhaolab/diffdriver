#' Title
#'
#' @param binary indicates the type of E variable
...
power_comparediff <- function(binary, Niter, sgdata, Nsample,para,bmrpars,betaf0,beta_gc,hot=0,hmm){
	...
	for (iter in 1:Niter) {
		...
		ef <- simdata$efsize
		fe <- c(ef$avbetaf1, ef$avbetaf1 + ef$avbetaf2)[as.factor(funcv)]+hotsize
		mr <- bmrmtx + ef$betaf0
		if (sum(mut) ==0) {next}
...
#' and the parameters used in these models.
#' @export
power_comparediff_dev <- function(gene,funcanno,Afiledir,binary,  para,Nsample, betaf0,  beta_gc,Niter){
  m1.pvalue <-  rep(1,Niter)
  a=c()
  b=c()
  for (iter in 1:Niter) {
    simdata <- simulate_dev(gene,funcanno,Afiledir,binary,  para,Nsample, betaf0,  beta_gc)
    ssgdata=simdata$annodata
    mut <- do.call(rbind, simdata$mutlist)
    bmrmtx <- do.call(rbind, simdata$bmrmtxlist)
    e <- simdata$pheno
