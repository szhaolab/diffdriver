...
With signature adjustment (`BMRmode = "signature"`), which is the default mode, it will need around 20min to estimate parameters in background model. After that it takes around 5-10 min for two genes. Each additional gene will take ~ 3 minutes. Please use the provide the annotation folder with 96 annotation files when running with the "signature mode". 
```{r}
...
Without signature adjustment (`BMRmode = "regular"`), it will need around 20min to estimate parameters in background model. Then it takes around 5min for two genes. Each additional gene will take 2 minutes. You can use the provided the annotation folder with 9 annotation files or 96 annotations. The example below uses 9 annotation files.
```{r}
